...
function InputField({ label, id, type = 'text', value, onChange, placeholder, hint, rows }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      {rows ? (
        <textarea id={id} rows={rows} value={value} onChange={onChange} placeholder={placeholder} className={cn(shared, 'resize-y min-h-[120px]')} />
      ) : (
...
export default function Config() {
  ...
  const handleTestGemini = async () => {
    ...
    try {
      const { data } = await configApi.testGemini({ gemini_api_key: form.gemini_api_key, gemini_model: form.gemini_model })
      toast.success(`Gemini OK: "${data.response}"`)
    } catch (err) {
      toast.error(err.response?.data?.error || 'Test Gemini gagal')
    } finally {
  ...
  const handleTestTelegram = async () => {
    ...
    try {
      const { data } = await configApi.testTelegram({ telegram_token: form.telegram_token })
      toast.success(`Telegram OK: @${data.bot_username} (${data.bot_name})`)
    } catch (err) {
      toast.error(err.response?.data?.error || 'Test Telegram gagal')
    } finally {
  ...
  return (
    <div className="relative w-full pb-16">
      ...
      {loading ? (
        ...
        <div className="animate-fade-in space-y-6 sm:space-y-8 grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="bg-white/60 backdrop-blur-2xl rounded-3xl sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white overflow-hidden flex flex-col">
            <div className="px-5 sm:px-8 py-5 sm:py-6 border-b border-white/60 bg-white/40 flex items-center gap-4 sm:gap-5">
              ...
              {config?._has_telegram_token && <span className="ml-auto text-[8px] sm:text-[9px] font-black bg-emerald-50 text-emerald-600 px-2 sm:px-3 py-1.5 rounded-full uppercase tracking-widest border border-emerald-100 shadow-sm flex-shrink-0">Tersambung</span>}
            </div>
            <div className="p-5 sm:p-8 flex flex-col gap-6 flex-1 bg-gradient-to-b from-transparent to-slate-50/30">
              <InputField
                ...
                hint="Rahasia: Token bot yang didapatkan dari @BotFather."
              />
              <div className="mt-auto pt-2 sm:pt-4">
                <button
                  onClick={handleTestTelegram}
                  disabled={testingTelegram || !form.telegram_token}
                  className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-2xl text-[10px] sm:text-[11px] font-black border-2 border-blue-100 text-blue-600 hover:bg-blue-50 hover:border-blue-200 transition-all disabled:opacity-40 uppercase tracking-widest"
                >
                  {testingTelegram ? <><span className="w-4 h-4 border-2 border-blue-500/30 border-t-blue-500 rounded-full animate-spin" /> PINGING SERVER...</> : '🔗 Uji Koneksi Telegram'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/60 backdrop-blur-2xl rounded-3xl sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white overflow-hidden flex flex-col">
            <div className="px-5 sm:px-8 py-5 sm:py-6 border-b border-white/60 bg-white/40 flex items-center gap-4 sm:gap-5">
              ...
              <div>
                <p className="text-base sm:text-lg font-black text-slate-800 tracking-tight">AI Engine (Gemini)</p>
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Otak Pintar Asisten</p>
              </div>
              {config?._has_gemini_key && <span className="ml-auto text-[8px] sm:text-[9px] font-black bg-emerald-50 text-emerald-600 px-2 sm:px-3 py-1.5 rounded-full uppercase tracking-widest border border-emerald-100 shadow-sm flex-shrink-0">Tersambung</span>}
            </div>
            <div className="p-5 sm:p-8 flex flex-col gap-6 flex-1 bg-gradient-to-b from-transparent to-slate-50/30">
              <InputField
                ...
                placeholder="AIza..."
                hint="Rahasia: Kunci akses dari Google AI Studio."
              />
              <div className="flex flex-col gap-2.5">
              ...
              <div className="mt-auto pt-2 sm:pt-4">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-2xl text-[10px] sm:text-[11px] font-black border-2 border-emerald-100 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-200 transition-all disabled:opacity-40 uppercase tracking-widest"
                >
                  {testingGemini ? <><span className="w-4 h-4 border-2 border-emerald-500/30 border-t-emerald-500 rounded-full animate-spin" /> PINGING MODEL...</> : '🧪 Uji Respon AI'}
                </button>
              </div>
            </div>
          </div>

          {/* System Instruction (Full Width) */}
          <div className="bg-white/60 backdrop-blur-2xl rounded-3xl sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white overflow-hidden lg:col-span-2 flex flex-col">
            <div className="px-5 sm:px-8 py-5 sm:py-6 border-b border-white/60 bg-white/40 flex items-center gap-4 sm:gap-5">
              ...
              <div>
                <p className="text-base sm:text-lg font-black text-slate-800 tracking-tight">Doktrin Kepribadian (System Prompt)</p>
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Penentu Karakter AI</p>
              </div>
            </div>
